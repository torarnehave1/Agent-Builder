...
// Agent-Builder's typed AuthUser (lib/auth.ts) omits emailVerificationToken,
// so read the raw `user` blob directly — same payload VegvisrAgentChat reads
// for its X-API-Token. Self-contained on purpose: no change to shared auth types.
function readRawUser(): { email: string | null; role: string | null; token: string } {
  if (typeof window === 'undefined') return { email: null, role: null, token: '' };
  try {
    const parsed = JSON.parse(window.localStorage.getItem('user') || '{}');
    return {
      email: parsed?.email || null,
      role: parsed?.role || null,
      token: parsed?.emailVerificationToken || '',
    };
  } catch {
    return { email: null, role: null, token: '' };
  }
}

/**
 * System Owner "Login as…" control — ported verbatim from the vemotion app.
 *
