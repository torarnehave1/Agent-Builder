  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        use_pages_token: { type: 'boolean', description: 'For the "cloudflare" server\'s execute tool ONLY, and only when founder_email is omitted (platform account). Set true to use CF_PAGES_TOKEN instead of the default CF_API_TOKEN — required for Cloudflare Pages project creation (POST /accounts/<id>/pages/projects) and Pages custom-domain attachment (POST /accounts/<id>/pages/projects/<name>/domains), since the default token does not have Pages permission. Verified working: cloudflare.request({method:"POST", path:`/accounts/${accountId}/pages/projects`, body:{name, production_branch, source:{type:"github", config:{owner, repo_name, production_branch}}}}) creates a project connected to an existing GitHub repo (no manual OAuth step needed — the repo just needs to exist), and a second call to `/accounts/${accountId}/pages/projects/<project_name>/domains` with body {name: "<custom-domain>"} attaches a custom domain in the same account (Pages: Edit alone was sufficient, no Zone permission needed). Do not set this for zones/DNS/Workers calls — those still need the default token.' }
      },
      required: ['server']
    }
  },
