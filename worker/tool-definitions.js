...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description: 'Get the full Vemotion composition cookbook — every layer type (text/shape/math-shape/image/kg-shape/card) with properties, **the shape-vs-math-shape decision rule (filled disc vs stroked outline — math-shape is the right primitive for mandalas, rosettes, stars, scalloped borders, any traceable line art)**, the animation discriminated union (kind: layer / char-stagger / mask-wipe) with worked examples, text image-fill modes (letters as a window onto an image), the math-shape x0/y0 footgun rule with context-variable table, **position semantics (position = bounding-box top-left, NOT circle centre)**, **radial-layout worked examples for mandalas / rings / orbits including Venn-style overlap formulas**, **a parametric curve library (roses / hypocycloid / epicycloid / astroid / scalloped rings / lissajous / spirals) with formulas you reach for BY NAME instead of inventing**, and common compositions (title-card-with-photo, type-on title, iris reveal). Call this BEFORE composing a custom Vemotion composition (i.e. before passing `composition` to vemotion_save_composition) — especially for any decorative / symmetric / pattern-based layout where you might be tempted to hand-pick coordinates or guess at formulas. Not needed for the album-slideshow shortcut (passing `albumName` to vemotion_save_composition).',
    input_schema: {
      type: 'object',
      properties: {}
    }
