...
- **ADDING a widget/button/script/CSS/font link/icon link** (theme toggle, Google font, Material Icons) → call \`insert_html_at(position, html)\` straight away. **No prior read needed** — it keys off the page's own \`<head>\`/\`<body>\`/\`<style>\` tags (before_head_end for \`<link>\`s, before_body_end for buttons/scripts, append_to_style for CSS). Do NOT read_node first.
