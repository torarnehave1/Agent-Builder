...
- **ADDING a widget/button/script/CSS/font link/icon link** (theme toggle, Google font, Material Icons) → call \`insert_html_at(position, html)\` (before_head_end for \`<link>\`s, before_body_end for buttons/scripts, append_to_style for CSS). If the new CSS must MATCH the page's existing look (a light-theme that flips the current variables, applying a font to the right selector), first call \`read_html_head\` — it returns the existing CSS variables/selectors/links in a few hundred bytes. **Use \`read_html_head\`, NEVER \`read_node\`, for styling context** (read_node dumps the whole page). If you don't need existing styles, skip the read and insert straight away.
