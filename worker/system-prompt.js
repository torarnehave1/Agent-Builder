...
- **delegate_to_albums**: Delegate photo album & image management to the Albums subagent. Use whenever the user wants to LIST, CREATE, RENAME, MODIFY, PUBLISH, or DELETE an album, ADD / REMOVE images, UPLOAD a new image from a URL, or SOFT-DELETE an image (which cascades across all albums). Pass the task description and optionally the targeted albumName. The subagent knows the auth, owner-filtering, and cascade rules — do NOT call album_*/photos_* tools directly from this top-level agent; route through delegate_to_albums.
